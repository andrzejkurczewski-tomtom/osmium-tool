...
#include "strategy_complete_ways_by_first_node_and_tags.hpp"
...
namespace strategy_complete_ways_by_first_node_and_tags {
    ...
    Strategy::Strategy(const std::vector<std::unique_ptr<Extract>>& extracts, const osmium::Options& options) {
        ...
        for (const auto& option : options) {
            if (option.first != "relations") {
                warning(std::string{"Ignoring unknown option '"} + option.first + "' for 'complete_ways_by_first_node_and_tags' strategy.\n");
            }
        }
    ...
    const char* Strategy::name() const noexcept {
        return "complete_ways_by_first_node_and_tags";
    }
    ...
    void Strategy::run(osmium::VerboseOutput& vout, bool display_progress, const osmium::io::File& input_file) {
        if (input_file.filename().empty()) {
            throw osmium::io_error{"Can not read from STDIN when using 'complete_ways_by_first_node_and_tags' strategy."};
        }

        vout << "Running 'complete_ways_by_first_node_and_tags' strategy in two passes...\n";
